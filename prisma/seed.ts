...
const createBurgers = async (
  ...
  const burgersCategory = await prismaClient.category.create({
    data: {
      name: "Hambúrgueres",
      imageUrl:
  ...
  for (const item of burgerRestaurants) {
...
const main = async () => {
  ...
  await createBurgers(desertsCategory.id, juicesCategory.id);
